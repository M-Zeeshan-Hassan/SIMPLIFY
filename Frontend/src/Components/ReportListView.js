...
import { purchaseInvoiceViewHeader, saleInvoiceViewHeader } from "../Pages/Dashboard/Sale/Data.js";


const ReportlistView = (WrappedComponent) => {

    return function ReportListViewComponent() {
        ...
        const currentConfig =
            activeTab === "Purchase"
                ? {
                    url: "http://localhost:8000/reports/invoice/purchases",
                    title: "Purchase Invoice Report",
                    tableHeader: purchaseInvoiceViewHeader,
                    path: "/purchases/pi/new",
                }
                : {
                    url: "http://localhost:8000/reports/invoice/sales",
                    title: "Sales Invoice Report",
                    tableHeader: saleInvoiceViewHeader,
                    path: "/sales/si/new",
                };



        const { getFetch } = useFetch(currentConfig.url);
        ...
        const { data: allData, isLoading, error } = useQuery(
            ['fetchAllData', activeTab, searchTerm, pageNo, perPage],
            async () => {
        ...
        return (
            <>
                ...
                <div className='min-w-max w-full'>
                    ...
                    <div className="flex justify-start items-center gap-2 mb-4 text-sm font-semibold">
                        <span
                            onClick={() => setActiveTab("Purchase")}
                            className={`inline-block pb-[6px] cursor-pointer font-semibold text-sm ${activeTab === "Purchase"
                                ? "text-blue-600 border-b-2 border-blue-600"
                                : "text-[#a5adba] hover:text-blue-600"
                                }`}
                        >
                            Purchase
                        </span>
                        ...
                        <span
                            ...
                            className={`inline-block pb-[6px] cursor-pointer font-semibold text-sm ${activeTab === "Sales"
                                ? "text-blue-600 border-b-2 border-blue-600"
                                : "text-[#a5adba] hover:text-blue-600"
                                }`}
                        >
              ...
              <p style={{ boxShadow: "0 0 6px #172b4d0a" }} className='flex px-2.5 items-center w-full justify-between text-textColor2'>
                {downArrow}
                            ...
                            <button
                                onClick={() => navigate(currentConfig?.path)}
                                type="button"
                    ...
                    <div>
                        <table style={{ boxShadow: '0 0 6px #172b4d2e' }} className='border rounded table border-collapse w-full'>
                            <thead className='bg-darkBlue'>
                                <tr>
                                    {currentConfig?.tableHeader.map((head, index) => (
