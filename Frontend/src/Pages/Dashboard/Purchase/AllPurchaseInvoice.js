...
import { PurchaseInvoiceHeader } from './Data.js'

const AllPurchaseInvoice = ({ searchTerm, allData, lengthData }) => {
          ...
          filteredData?.map((field) => (
            <tr key={field._id} className='  hover:bg-searchIcon border border-b-2 border-b-searchIcon '>
              ...
              <td className="px-2.5 py-3 text-xs font-semibold text-textColor whitespace-nowrap">
                <Link to={`/purchases/pi/edit/${field._id}`} className="hover:underline">
                  {field?.SINumber}
                </Link>
              </td>
              ...
              <td className='p-3 font-semibold text-textColor2 text-xs'>{formatDate(field?.createdAt)}</td>
               ...
               <td >
                <Link to={`/team/view/${field?.supplierDetails?.SupplierId}`} style={{ fontSize: '12.5px' }}
               ...
               <td className='p-3 font-medium text-textColor2 text-xs'>
                {field?.invoiceDetails?.purchaseRefernce}
              </td>
              ...
              <td >
                <Link to={`/team/view/${field?.invoiceDetails?.assigneTo?.id}`} style={{ fontSize: '12.5px' }}
                  className='p-3   text-textColor2 hover:underline hover:text-textColor'>
              ...
              <td className="px-1 py-3 text-center whitespace-nowrap">
                <span className="text-[12.5px] bg-bgColor text-white rounded px-1.5 py-1 block w-fit ">
                  {field?.Status}
                </span>
              </td>
              ...
              <td style={{ fontSize: '12.5px' }} className="p-3 font-medium text-textColor2  relative group">
                ...
                <div className="absolute  top-4 right-24 w-24  p-1 bg-white rounded shadow-sideShadow hidden group-hover:block z-10">
                  <Link
                    to={`/purchases/pi/edit/${field._id}`} style={{ fontSize: '12.5px' }}
...
export default listView(AllPurchaseInvoice, "http://localhost:8000/purchases/pi/list","Purchase Invoices", PurchaseInvoiceHeader,'/purchases/pi/new')
