...
const DashboardLayout = () => {
    ...
    return (
        <div className="smiels-dashboard flex relative h-screen dashboard-root">
            <Sidebar sideOpen={sidebarOpen.sideOpen} />

            <div style={{ marginTop: '60px' }} className="main block w-full">
                <Header
                    toggleSidebar={toggleSidebar}
                    menuClose={sidebarOpen.menuClose}
                    menuOpen={sidebarOpen.menuOpen}
                />

                <div
                    style={{ height: 'calc(100vh - 60px)', padding: '30px' }}
                    className="containers overflow-y-auto dashboard-content"
                >
                    <div className="overflow-x-auto dashboard-interactive">
                        <Outlet />
                    </div>
                </div>
            </div>
